...
def run_grover(cfg: GroverConfig) -> Counts:  # pragma: no cover - exercised in integration tests
    try:
        _require_qiskit(use_runtime=False)
    except ImportError:
...
def energy_aware_search(
    ...
    *,
    ...
    confidence_floor = min_shots_for_confidence(eps=eps, delta=delta)
    shots = max(confidence_floor, max(1, cfg.shots))
    shots_used = 0
    counts: Counts = {}
    success = 0.0
    ...
    while True:
        ...
        next_shots = max(shots * 2, confidence_floor)
        shots = min(next_shots, max_shots_cap)
