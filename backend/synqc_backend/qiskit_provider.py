    ...
    def run(self, preset: ExperimentPreset, shot_budget: int) -> ProviderLiveResult:
        ...
        try:
            ...
            if preset is ExperimentPreset.GROVER_DEMO:
                ...
                success = 0.0
                total_shots_used = 0
                remaining_budget = shots
                while remaining_budget > 0:
                    current_shots = min(initial_cfg.shots, remaining_budget)
                    ...
                    shots_consumed = result.shots_used or current_shots
                    total_shots_used += shots_consumed
                    remaining_budget = max(0, shots - total_shots_used)
                    success = grover_utils.success_probability(counts=counts, marked={"11"})
                    if success >= 0.9:
                        break
                    if remaining_budget <= 0:
                        break
                    next_shots = min(current_shots * 2, shots)
                    initial_cfg = initial_cfg.with_shots(next_shots)
