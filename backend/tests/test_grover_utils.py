...
def test_energy_aware_stops_on_early_success(monkeypatch):
    shot_sequence: list[int] = []
    ...
    cfg = GroverConfig(n_qubits=3, marked=["101"], shots=8, seed_sim=None)
    shots_used, counts, success = grover.energy_aware_search(
        cfg, target_success=0.8, max_shots_cap=64, verbose=False
    )

    assert len(shot_sequence) == 2
    assert shots_used < 64
    assert success >= 0.8
    assert sum(counts.values()) == shots_used
    assert all(s <= 64 for s in shot_sequence)


def test_energy_aware_respects_tight_cap(monkeypatch):
    shot_sequence: list[int] = []
    ...
    tight_cap = 10
    cfg = GroverConfig(n_qubits=3, marked=["101"], shots=32, seed_sim=None)
    shots_used, counts, success = grover.energy_aware_search(
        cfg, target_success=0.8, max_shots_cap=tight_cap, verbose=False
    )

    assert all(s <= tight_cap for s in shot_sequence)
    assert shots_used <= tight_cap
    assert success >= 0.8
    assert sum(counts.values()) == shots_used


def test_energy_aware_respects_confidence_floor(monkeypatch):
    shot_sequence: list[int] = []
    ...
    shots_used, counts, success = grover.energy_aware_search(
        ...
        eps=0.2,
        delta=0.2,
        max_shots_cap=confidence_floor + 5,
        verbose=False,
    )

    assert shot_sequence[0] == confidence_floor
    assert shots_used == confidence_floor
    assert success >= 0.9
    assert sum(counts.values()) == shots_used


def test_energy_aware_confidence_floor_respects_cap(monkeypatch):
    shot_sequence: list[int] = []
    confidence_floor = grover.min_shots_for_confidence(eps=0.2, delta=0.2)
    ...
    shots_used, counts, success = grover.energy_aware_search(
        ...
        eps=0.2,
        delta=0.2,
        max_shots_cap=tight_cap,
        verbose=False,
    )

    assert shot_sequence == [tight_cap]
    assert shots_used == tight_cap
    assert success == 0.0
    assert sum(counts.values()) == shots_used
