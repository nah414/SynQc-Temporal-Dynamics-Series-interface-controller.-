// --------------------------------------------
    ...
    const hardwareButtons = document.getElementById('hardwareButtons');
    ...
    const backendSyncMeta = document.getElementById('backendSyncMeta');
    ...
    let backendApiVersion = null;
    let lastBackendRefresh = null;
    ...
    function updateBackendMeta({ serverTime } = {}) {
      if (!backendSyncMeta) return;

      if (!backendReachable) {
        backendSyncMeta.textContent = `Backend offline — ${API_BASE}`;
        return;
      }

      const parts = [];
      if (backendApiVersion) parts.push(`API ${backendApiVersion}`);
      if (hardwareTargetsCache?.length) parts.push(`${hardwareTargetsCache.length} targets`);
      if (lastBackendRefresh) parts.push(`last sync ${fmtShortTime(lastBackendRefresh)}`);
      if (serverTime instanceof Date && !Number.isNaN(serverTime.getTime())) {
        parts.push(`server ${fmtShortTime(serverTime)} UTC`);
      }

      if (parts.length === 0) {
        backendSyncMeta.textContent = `Connected to ${API_BASE}`;
      } else {
        backendSyncMeta.textContent = `Connected · ${parts.join(' · ')}`;
      }
    }
    ...
    function setActiveHardwareButton(value) {
      if (!hardwareButtons) return;
      Array.from(hardwareButtons.querySelectorAll('.hardware-pill')).forEach(btn => {
        btn.classList.toggle('active', btn.dataset.value === value);
      });
    }

    function rebuildHardwareButtons() {
      if (!hardwareButtons || !hardwareSelect) return;
      while (hardwareButtons.firstChild) hardwareButtons.removeChild(hardwareButtons.firstChild);

      Array.from(hardwareSelect.options || []).forEach(opt => {
        const btn = document.createElement('button');
        btn.type = 'button';
        btn.className = 'hardware-pill';
        btn.dataset.value = opt.value;
        btn.textContent = opt.textContent;
        btn.addEventListener('click', () => {
          hardwareSelect.value = opt.value;
          hardwareSelect.dispatchEvent(new Event('change'));
          setActiveHardwareButton(opt.value);
        });
        hardwareButtons.appendChild(btn);
      });

      setActiveHardwareButton(hardwareSelect.value);
    }
    ...
    function updateRunAvailability() {
      const hasHardware = Boolean(hardwareSelect?.value);
      const shouldDisable = !backendReachable || !hasHardware;
      runPresetBtn.disabled = shouldDisable;

      if (!backendReachable) {
        setRunStatus(`Backend: not reachable (${API_BASE}). Start it and refresh.`);
      } else if (!hasHardware) {
        setRunStatus('Select a hardware target before running.');
      }
    }
    ...
    async function refreshFromBackend() {
      ...
      try {
        ...
        backendApiVersion = h?.api_version || null;
        lastBackendRefresh = new Date();
        const serverTime = h?.server_time_utc ? new Date(h.server_time_utc) : null;
        ...
        try {
          ...
          } else if (hardwareSelect.options.length > 0) {
            hardwareSelect.selectedIndex = 0;
          }
          rebuildHardwareButtons();
          hardwareSelect.dispatchEvent(new Event('change'));
    ...
    rebuildHardwareButtons();
    refreshFromBackend();
    refreshIntervalId = setInterval(() => refreshFromBackend(), REFRESH_INTERVAL_MS);
    updateInterpretationText();
